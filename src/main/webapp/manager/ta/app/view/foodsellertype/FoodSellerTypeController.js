 ...
 */
Ext.define('TA.view.foodsellertype.FoodSellerTypeController', {
    ...
    alias: 'controller.foodsellertype',
    stores: ['FoodSellerType'],
    views: ['foodsellertype.List'],
    refs: [{
        ref: 'foodsellertypelist',
        selector: 'foodsellertypelist'
    }],
    init: function() {
    	console.log('FoodSellerTypeController init.');
    	this.foodsellertypelist = this.getView();
    	console.log('get view: ' + this.foodsellertypelist);
		
	},
	onAdd: function(button, e) {
		console.log('onAdd ' + button + " " + e);
		var window = Ext.widget('foodsellertypeedit');
	...
	add: function(button, e) {
		...
		var store = Ext.getStore('FoodSellerType');
		/*if (form.isValid()) {
		...
		form.submit({
			url: '../../manager/foodSellerType/add',
			method: 'POST',
			/*params: params,*/
			/*waitTitle: 'Waiting',
			waitMsg: 'Waiting...',*/
			success: function(form, action) {
				...
				store.insert(0, new TA.model.FoodSellerType());
	...
	update: function(button, e) {
		...
		form.submit({
			url: '../../manager/foodSellerType/update',
			method: 'POST',
			/*params: params,*/
			/*waitTitle: 'Waiting',
			waitMsg: 'Waiting...',*/
	...
	deleteRecord: function(records) {
        ...
        Ext.each(records, function(record, index) {
        	ids.push(record.get('foodSellerTypeId'));
        });
        
        Ext.Ajax.request({
            url: '../../manager/foodSellerType/delete',
            params: {ids:ids},
            method: 'POST',
