 ...
 */
Ext.define('TA.view.recommend.RecommendController', {
    ...
    stores: ['Recommend'],
    views: ['recommend.List'],
    refs: [{
        ref: 'recommendlist',
        selector: 'recommendlist'
    }],
    init: function() {
    	console.log('RecommendController init.');
    	this.recommendlist = this.getView();
    	console.log('get view: ' + this.recommendlist);
		
	},
	onAdd: function(button, e) {
		console.log('onAdd ' + button + " " + e);
		var window = Ext.widget('recommendedit');
	...
	add: function(button, e) {
		...
		var store = Ext.getStore('Recommend');
		/*if (form.isValid()) {
		...
		form.submit({
			...
			success: function(form, action) {
				...
				store.insert(0, new TA.model.Recommend());
	...
	deleteRecord: function(records) {
        ...
        Ext.each(records, function(record, index) {
        	ids.push(record.get('recommendId'));
        });
        
        Ext.Ajax.request({
            url: '../../mgr/recommend/delete',
            params: {ids:ids},
            method: 'POST',
	...
	onSearch: function(button, e) {
		...
		name = Ext.ComponentQuery.query('toolbar textfield[name=title]', view)[0].getValue();
		var params = {
				title: name
		};
