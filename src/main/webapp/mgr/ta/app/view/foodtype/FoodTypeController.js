 ...
 */
Ext.define('TA.view.foodtype.FoodTypeController', {
    ...
    alias: 'controller.foodtype',
    stores: ['FoodType'],
    views: ['foodtype.List'],
    refs: [{
        ref: 'foodtypelist',
        selector: 'foodtypelist'
    }],
    init: function() {
    	console.log('FoodTypeController init.');
    	this.foodtypelist = this.getView();
    	console.log('get view: ' + this.foodtypelist);
		
	},
	onAdd: function(button, e) {
		console.log('onAdd ' + button + " " + e);
		var window = Ext.widget('foodtypeedit');
	...
	add: function(button, e) {
		...
		var store = Ext.getStore('FoodType');
		/*if (form.isValid()) {
		...
		form.submit({
			...
			success: function(form, action) {
				...
				store.insert(0, new TA.model.FoodType());
	...
	deleteRecord: function(records) {
        ...
        Ext.each(records, function(record, index) {
        	ids.push(record.get('foodTypeId'));
        });
        
        Ext.Ajax.request({
            url: '../../mgr/foodType/delete',
            params: {ids:ids},
            method: 'POST',
