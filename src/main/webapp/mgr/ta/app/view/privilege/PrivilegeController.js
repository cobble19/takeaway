 ...
 */
Ext.define('TA.view.privilege.PrivilegeController', {
    ...
    alias: 'controller.privilege',
    stores: ['Privilege'],
    views: ['privilege.List'],
    refs: [{
        ref: 'privilegelist',
        selector: 'privilegelist'
    }],
    init: function() {
    	console.log('PrivilegeController init.');
    	this.privilegelist = this.getView();
    	console.log('get view: ' + this.privilegelist);
		
	},
	onAdd: function(button, e) {
		console.log('onAdd ' + button + " " + e);
		var window = Ext.widget('privilegeedit');
	...
	add: function(button, e) {
		...
		var store = Ext.getStore('Privilege');
		/*if (form.isValid()) {
		...
		form.submit({
			...
			success: function(form, action) {
				...
				store.insert(0, new TA.model.Privilege());
	...
	onUpdate: function(button, e) {
		...
		window.down('form').add(this.createRole(record.get('privilegeId')));
		window.down('form').updateLayout();
	},
	...
	onSearch: function(button, e) {
		...
		name = Ext.ComponentQuery.query('toolbar textfield[name=privilegeName]', view)[0].getValue();
		var params = {
				privilegeName: name
		};
	...
	createRole: function(privilegeId) {
		console.log('create Role Checkboxgroup: ' + privilegeId);
		var url = '../../mgr/role/all';
		if (privilegeId == undefined || privilegeId == null) {
			url = '../../mgr/role/all';
		} else {
			url = '../../mgr/role/' + privilegeId;
		}
