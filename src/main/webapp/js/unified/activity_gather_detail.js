...
var needSubscribe = function() {
	...
	var activityGatherId = getParam('activityGatherId');
	var hidContent = getParam('hidContent');
	$.ajax({
		"url" : $('#basePath').val() + "/api/unified/activitygather/" + activityGatherId,
		"type" : "GET",
		"headers" : {
...
//    	params.apply2AttrModelName = $(inputText).prev().prev().prev().children('span').html();
    	...
    	params.orderNo = 1;*/
    	params.activityGatherId = activityGatherId;
...
//		    var userId = $('#userId').val();
		    ...
		    for (var i = 0; i < inputTexts.length; i++) {
		    	...
		    	apply2AttrPOJO.activityGatherId = activityGatherId;
...
//        	$("#authorizerAppId").html(data.wxAuthorizerInfoPOJO.authorizerAppId);
        	$("#activityGatherId").html(data.activityGatherId);
