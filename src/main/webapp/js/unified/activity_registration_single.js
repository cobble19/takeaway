var table4ActivityRegistration;
$(document).ready(function() {

    table4ActivityRegistration = $('#dbTable4ActivityRegistration').DataTable( {
    	"processing": true,
...
//		}, {
			...
			"targets" : 9,
			"render" : function(data, type, full, meta) {
				/*var href = $('#basePath').val() + '/page/person/apply_in_activity.jsp?activityRegistrationId='  + full.activityRegistrationId
				...
				var hrefEdit = $('#basePath').val() + '/page/unified/activity_registration_update.jsp?activityRegistrationId='  + full.activityRegistrationId;
				...
				var url = $('#basePath').val() + '/page/unified/activity_registration_detail.jsp?activityRegistrationId=' + full.activityRegistrationId + "&hidContent=1&a=1";
				...
				var linkPublish = '<a data-toggle="tooltip" data-placement="top" title="' + publishTypeTitle + '"' +
						' class="btn btn-warning btn-xs picBtn" style="margin-bottom:5px;" href="#" onclick="changePublish4ActivityRegistration(' + full.activityRegistrationId + ',' + full.publishType + '); return false;">' 
//						+ publishType 
...
//            { "data": "typeCode" },
    ...
    table4ActivityRegistration.on( 'order.dt search.dt', function () {
    	table4ActivityRegistration.column(1, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
            cell.innerHTML = i+1;
        } );
    } ).draw();
    ...
    /*$('a.picBtn').click(function(e) {
    	e.preventDefault();
    	console.log($(this));
    	data = table4ActivityRegistration.row($(this).parent('tr')).data();
    	console.log(data);
    });*/
	...
	$('#picForm').find('#uploadBtn').click(function(e) {
	    ...
	    formData.append('wxTemplateId', $('#activityRegistrationId').val());
	...
	/*$('#picForm').find('#addBtn').click(function(e) {
		...
		var params = {
				userId: $('#userId').val(),
				logoImg: $('#logoImg').val(),
				activityRegistrationId: $('#activityRegistrationId').val()
		};
		$.ajax({
    ...
    $('#chkBoxAll').click(function() {
    	...
    	if (chkBoxAll) {
    		$('#dbTable4ActivityRegistration').find('input[name=chkBox]').attr('checked', true);
    	} else {
    		$('#dbTable4ActivityRegistration').find('input[name=chkBox]').attr('checked', false);
    	}
    })

    $('#deleteBtn').click(function() {
    	var ids = [];
    	var chkBox = $('#dbTable4ActivityRegistration').find('input[name=chkBox]');
    	
    	chkBox.each(function(index, ele) {
//    		console.log($(this).val() + ele.value);
        	/*var tr = $(this).closest('tr');
            var row = table4ActivityRegistration.row( tr );*/
    		if ($(this).attr('checked')) {
//    			ids.push(row.data().activityRegistrationId);
    			ids.push($(this).val());
    		}
    	})
    	console.log('ids: ' + ids);
    	...
    	$.ajax({
    		"url" : "../../mgr/activityregistration/delete",
    		"type" : "GET",
    		/*"headers" : {
    			"Content-Type" : "application/json"
    		},*/
            ...
            success: function(data, textStatus, jqXHR ) {
            	$('#progress').dialog('close');
            	activityRegistrationSearch(table4ActivityRegistration);
            },
            error: function(jqXHR, textStatus, errorThrown) {
            	console.log('Load Error!');
            },
    ...
    $('#showLinkBtn4ActivityRegistration').click(function() {
    	openActivityRegistrationActiveListDiv();
    });
    
    $('#activityRegistrationDetailDiv').dialog({
    	autoOpen: false,
    	modal: true,
    	width: 660,
    	height: 150
    ...
    $('#activityRegistrationActiveListDiv').dialog({
    	autoOpen: false,
    	modal: true,
    	width: 660,
    	height: 150
    ...
    deleteAttrModel();
    /// 1
    var clipboard = new Clipboard('#copyBtn4ActivityRegistration');
...
//        e.clearSelection();
    ...
    ///  2
    var clipboard2 = new Clipboard('#copyBtn24ActivityRegistration');
...
//        e.clearSelection();
    ...
    $('#searchBtn4ActivityRegistration').click(function() {
    	console.log('search click...');
    	activityRegistrationSearch(table4ActivityRegistration);
    })
     
    // Add event listener for opening and closing details
    $('#activityRegistrationList tbody').on('click', 'td.details-control', function () {
        var tr = $(this).closest('tr');
        var row = table4ActivityRegistration.row( tr );
...
function format ( d ) {
    ...
    return '<form id="applyForm" class="form-inline">' +
    		'<input type="hidden" id="activityRegistrationId" name="activityRegistrationId" value="' + d.activityRegistrationId + '"/>' + 
...
var activityRegistrationSearch = function(table) {
	$('#progress').dialog('open');
	var params = {
//            typeCode: $("#typeCode option:selected").val()
	}
	$.ajax({
		"url" : "../../api/unified/activityRegistrationByUserId",
		"type" : "POST",
		/*"headers" : {
			"Content-Type" : "application/json"
		},*/
...
var openUrlDivII = function(url) {
	$('#activityRegistrationDetailDiv').dialog('open');
	
	$('#activityRegistrationDetailUrl').val(url);
	$('#activityRegistrationDetailUrl2').val(url.replace('hidContent=1', 'hidContent=0'));
...
var openPicDiv = function(t) {
	data = $('#dbTable4ActivityRegistration').DataTable().row($(t).closest('tr')).data();
	console.log(data);
	$('#activityRegistrationId').val(data.activityRegistrationId);
...
var openApply2AttrModelDiv = function(t) {
	data = $('#dbTable4ActivityRegistration').DataTable().row($(t).closest('tr')).data();
	console.log(data);
	$('#apply2AttrModelForm').find('#activityRegistrationId').val(data.activityRegistrationId);
...
function showApply2AttrModel() {
	var activityRegistrationId = $('#apply2AttrModelForm').find('#activityRegistrationId').val();
...
function deleteAttrModel() {
	$('#deleteAttrBtn').click(function() {
		...
		$.ajax({
			"url" : $('#basePath').val() + "/api/apply2AttrModel/delete/" + activityRegistrationId,
			"type" : "GET",
			/*"headers" : {
				"Content-Type" : "application/json"
			},*/
...
//				alert('请正确输入');
		    ...
		    var userId = $('#userId').val();
		    var activityRegistrationId = $(form1).find('#activityRegistrationId').val();
		    ...
		    for (var i = 0; i < inputTexts.length; i++) {
		    	...
		    	apply2AttrModelPOJO.activityRegistrationId = activityRegistrationId;
...
/// changePublish4ActivityRegistration
var changePublish4ActivityRegistration = function(activityRegistrationId, publishType) {
	// 取反
...
//        	alert('更新成功, activityRegistrationId: ' + activityRegistrationId);	
...
/// end changePublish4ActivityRegistration
